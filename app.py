...
def normalize_greeting(message):
    ...
    if "good afternoon" in text or "굿애프터눈" in text or "굿 애프터눈" in text:
        ...
        greeting = "Hey"
    elif re.search(r"(?:^|\s)(?:hello|hallo|halo|yellow)(?:\s|$)", text) or "헬로" in text or "안녕" in text:
        greeting = "Hello"
    else:
...
def chatbot_config():
    ...
    return jsonify({
        ...
        "introTtsParts": [{"text": part, "token": make_tts_token(part)} for part in intro_speech_parts()],
    })
...
def question_retry_response(stage, original, ambiguous=False):
    ...
    if ambiguous and attempt < 3:
        return respond("Card or cat? Please say it again.", "다시 단어를 또박또박 말해 보세요!", stage, original=original, corrected="")
    if attempt == 1:
    ...
    if stage in default_examples:
        ...
        return respond(f'Let\'s try together. "{example}"', "화면의 문장을 천천히 따라 말해 보세요!", stage, original=original, corrected="", speech_reply=f"Let's try together. {example}")
    return respond("Choose an easy item. Try again!", "내가 발음하기 쉬운 물건으로 시도해보세요!", stage, original=original, corrected="")
...
def chat():
    ...
    if stage != Stage.WAIT_GREETING.value and re.search(r"[가-힣]", original):
        return respond("Please say that again in English.", "영어로 다시 한 번 말해 보세요.", stage, original=original, corrected="")
    ...
    question_stages = {
        ...
        Stage.STUDENT_QUESTION_2.value: (
            ...
            f"How about you? Do you have {CHARACTER.get('preference_item', 'a book')}?",
        ),
